...
#def get_dataframe(data):, 20, 21,
#    df = data

data = pd.merge(data, Weather, left_on=['Start'],  right_on=['Time'])
data.to_excel("data3.xlsx")    
feature_cols1 = [1, 3,  4, 5, 6,  9, 10, 11, 12, 13, 14, 15, 18, 19, 20, 21, 25, 26, 27, 29, 30, 31, 32, 33, 34]
class_cols = [2]
...
R = cross_val_score(reg, X, y, cv=5, scoring='r2')
#R2 = 1-(1-mean(R)*(len(y)-1))/(len(y)-28-1)
MAE = cross_val_score(reg, X, y, cv=5, scoring='neg_mean_absolute_error')
RMSE = cross_val_score(reg, X, y, cv=5, scoring='neg_root_mean_squared_error')
MAPE = cross_val_score(reg, X, y, cv=5, scoring='neg_mean_absolute_percentage_error')
print('R cross val:',mean(R))
#print(R2)
print('Mean Absolute Error cross val:', mean(MAE))
print('Root Mean Squared Error cross val:', mean(RMSE))
print('MAPE cross val:', mean(MAPE))
print('Adjusted R:',1 - (1-reg.score(X_test, y_test))*(len(y_test)-1)/(len(y_test)-X_test.shape[1]-1))

...
clf = SVR(kernel='rbf', C=1, gamma='auto', epsilon=0.1) # Linear Kernel
clf.fit(X, y)
y_pred1 = clf.predict(X_test)
R = cross_val_score(clf, X, y, cv=5, scoring='r2')
#R2 = 1-(1-mean(R)*(len(y)-1))/(len(y)-28-1)
MAE = cross_val_score(clf, X, y, cv=5, scoring='neg_mean_absolute_error')
RMSE = cross_val_score(clf, X, y, cv=5, scoring='neg_root_mean_squared_error')
MAPE = cross_val_score(clf, X, y, cv=5, scoring='neg_mean_absolute_percentage_error')
...
print('Adjusted R:',1 - (1-clf.score(X_test, y_test))*(len(y_test)-1)/(len(y_test)-X_test.shape[1]-1))
